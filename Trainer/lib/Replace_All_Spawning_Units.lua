...
if next( replace_units_idstring ) ~= nil then
	Hooks:PostHook(GroupAITweakData, "_init_unit_categories", "Replace_All_Spawning_Units", function(gai, difficulty_index, ...)
		for k, v in pairs(gai.unit_categories or {}) do
			...
			if v.unit_types and v.unit_types.america and v.unit_types.russia and v.unit_types.murkywater and k ~= "Phalanx_vip" and k ~= "Phalanx_minion" then
				gai.unit_categories[k].unit_types.america = replace_units_idstring
				gai.unit_categories[k].unit_types.russia = replace_units_idstring
				gai.unit_categories[k].unit_types.murkywater = replace_units_idstring
			end
		end
	end)
end
