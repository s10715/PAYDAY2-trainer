local function revive_teammates()
	...
	if state == "arrested" or state == "bleed_out" or state == "fatal" or state == "incapacitated" then
	...
	for _,unit in pairs(managers.interaction._interactive_units) do
		if tostring(unit:name()) ~= tostring(player:name()) and unit:interaction().tweak_data == "free" or unit:interaction() and unit:interaction().tweak_data == "revive" then
