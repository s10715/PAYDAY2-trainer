...
-- host only (state won't affect bots, except our custom state)
-- state can be one of those string value: standard, arrested, bleed_out, fatal, tased, incapacitated, and some custom string (won't affect yourself): kick, incustody, uncustody
...
local function change_player_state(state, peer_id)
	...
	-- custom states
	if state == "kick" and peer_id ~= managers.network._session:local_peer():id() then 
